package com.songlib.data.sample

import com.songlib.data.models.Song

val SampleSongs = listOf(
    Song(
        book = 1,
        songId = 1,
        songNo = 1,
        title = "Only Believe",
        alias = "",
        content = "Fear not, little flock,#from the cross to the throne,##CHORUS#Only believe, only believe,#All things are possible,",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 2,
        songNo = 2,
        title = "Amazing Grace",
        alias = "",
        content = "Amazing grace!#How sweet the sound##'Twas grace that taught#my heart to fear,#And grace my fears relieved;#How precious did#that grace appear",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 3,
        songNo = 3,
        title = "They Come",
        alias = "",
        content = "They come from the East and West,#They come from the lands afar,#To feast with the King, to dine as#His guest#How Blessed these pilgrims are!#Beholding His hallowed face#Aglow with light divine;#Blest partakers of His grace,#As gems in His crown to shine.##CHORUS#Since Jesus has set me free,#I’m happy as heart can be;#No longer I bear the burden of care,#His yoke is so sweet to me,#My soul was as black as night,#But darkness has taken flight;#Now I shout the victory,#For Jesus has set me free.##I look on the great white throne,#Before it the ransom’d stand;#No longer are tears, no sorrow is known#Nor death in that goodly land.#My Saviour has gone before,#Preparing the way for me;#Soon we’ll meet to part to more#Thru’ time or eternity.##The gates of that holy place#Stand open by night and day;#O look to the Lord  who  'giveth more grace,'#Whose love has prepared the way.#A home in those mansions fair#His hand hath reserved for all.#For the wedding feast prepare,#Obeying the gracious call.##Oh Jesus is coming soon,#Our trials will then be o’er#What if our Lord this moment should come#For those who are free from sin?#Then would it bring you joy,#Or sorrow and deep despair?#When our Lord in glory comes,#We’ll meet Him up in the air.",
        views = 0,
        likes = 0,
        liked = false,
        created = "2023-08-23T09:36:48.560Z"
    ),
    Song(
        book = 1,
        songId = 4,
        songNo = 4,
        title = "I Love Him",
        alias = "",
        content = "Gone from my heart the#world and all its charms;#Now, through the blood,#I’m saved from all alarms;",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 5,
        songNo = 5,
        title = "Sweet Hour Of Prayer",
        alias = "",
        content = "Sweet hour of prayer,#sweet hour of prayer,",
        views = 0,
        likes = 0,
        liked = false,
        created = "2023-08-23T09:36:48.561Z"
    ),
    Song(
        book = 1,
        songId = 6,
        songNo = 6,
        title = "Oh, How I Love Jesus",
        alias = "",
        content = "There is a name I love to hear,#I love to sing its worth;",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 7,
        songNo = 7,
        title = "When The Redeemed Gather",
        alias = "",
        content = "I am thinking of the rapture#In our blessed home on high,##CHORUS#When the redeemed are gathering in#Washed like the snow,",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 8,
        songNo = 8,
        title = "Oh, I Want To See Him",
        alias = "",
        content = "As I journey through the land,#singing as I go,#Pointing souls to Calvary,##CHORUS#Oh, I want to see Him,#look upon His face,",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 9,
        songNo = 9,
        title = "Feeling So Much Better",
        alias = "",
        content = "Feeling so much better#talking about this good old Way,#Feeling so much better",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    ),
    Song(
        book = 1,
        songId = 10,
        songNo = 10,
        title = "Teach Me Lord, To Wait",
        alias = "",
        content = "Teach me, Lord, to wait#down on my knees,#Till in Your Own good time##Chorus#They that wait upon the Lord,#shall renew their strength,",
        views = 0,
        likes = 0,
        liked = false,
        created = ""
    )
)
